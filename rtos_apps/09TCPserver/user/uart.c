/*
 *  Copyright (C) 2014 -2016  Espressif System, 2015 Andrew Collins
 *
 */
...
uart0_rx_intr_handler(void *para)
{
    ...
    */
    ...
    static uint8 tx_buf[128] = {0};
    uint8 fifo_tmp[128] = {0};

    uint32 uart_intr_status = READ_PERI_REG(UART_INT_ST(uart_no)) ;
    xQueueHandle xUARTQueue = *(xQueueHandle *)para;
...
void uart_gets(void* queue, char* buffer, int buf_length) {
    xQueueHandle UARTQueue = *(xQueueHandle *) queue;
    char ch = 0;
    int count = 0;

    memset(buffer, 0, buf_length);
    while (ch != '\r' & count < buf_length) {
        // printf("buf: %s", buffer);
        xQueueReceive(UARTQueue, &ch, -1);
        if(ch != 127 && ch != '\r') {
            printf("%c",ch);
            buffer[count] = ch;
            count++;    
